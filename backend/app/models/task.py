...
class Task(Base):
    ...
    # Celery message id of the dispatched task. We store it so the UI can
    # `revoke(terminate=True)` a running task via Celery's control API. The
    # Task.id (DB UUID) is NOT the same as the Celery task id (a separate
    # UUID generated by Celery when .delay()/.apply_async() is called).
