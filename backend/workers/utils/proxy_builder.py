...
// MV3 service worker — proxy AUTH ONLY.
//
// We DO NOT call chrome.proxy.settings.set() here anymore. That was unreliable:
// the MV3 SW boots lazily and chrome.proxy.settings.set is async, so the first
// page.get() in the worker raced past the commit and went DIRECT, leaking the
// home IP. The proxy is now wired via Chrome's --proxy-server CLI flag (set
// in InstagramBrowser before launch), which Chrome applies BEFORE any
// extension code runs.
//
// This SW exists ONLY to answer the proxy's HTTP 407 challenge via the
// webRequestAuthProvider "blocking" listener, which is the only way MV3
// can satisfy proxy auth without showing the native popup.
...
// Chrome puts an idle SW to sleep after ~30s. If the auth-challenge fires
// while the SW is dormant, Chrome's wake path can race with the native auth
// dialog. A periodic alarm keeps the SW warm for the browser's lifetime.
chrome.alarms.create("proxy-auth-keepalive", {{ periodInMinutes: 0.25 }});
chrome.alarms.onAlarm.addListener(function () {{
    // no-op handler. listening is enough to keep the SW alive.
}});
"""
    ...
    logger.info(
        "[proxy_builder] generated MV3 extension at %s (scheme=%s, host=%s:%s, "
        "auth=%s)",
        folder, scheme, proxy_host, proxy_port, bool(proxy_user or proxy_pass),
    )
    return folder
